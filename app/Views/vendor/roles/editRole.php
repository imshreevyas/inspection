                            ...
                            <div class="card-body">
                                ...
                                <h4 class="card-title">Basic Roles Details</h4>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-sm-10">
                                            <input class="form-control" name="roleName" type="text" placeholder="eg: jhonsmith" value="<?= $singleRole[0]['role_name'] ?>"  id="username">
                                        </div>
                                    </div>
                                    <hr>
                                    ...
                                    <div class="row mb-3">
                                        <?php if(count($vendorSidebar) > 0) { 
                                                foreach($vendorSidebar as $key=>$singleData){
                                                    $perms = json_decode($singleRole[0]['perms'],true);
                                        ?>
                                        <div class="col-sm-2" style="margin-bottom:15px;padding:10px;border-bottom:1px solid #363d4e">
                                            ...
                                            <?php if($singleData['add_access'] == 1){  ?>
                                                <label class="form-check">
                                                    <input type="checkbox" name="role[<?= $singleData['sidebar_url'] ?>][]" <?= isAuthorized('add', $singleData['sidebar_url'], $perms) ? 'checked' : '' ?> value="add" id="add" class="form-check-input">
                                                    <span class="badge badge-soft-success">Add Access</span>
                                                </label>
                                            <?php } if($singleData['view_access'] == 1){  ?>
                                                <label class="form-check">
                                                    <input type="checkbox" name="role[<?= $singleData['sidebar_url'] ?>][]" <?= isAuthorized('view', $singleData['sidebar_url'], $perms) ? 'checked' : '' ?> value="view" id="view" class="form-check-input">
                                                    <span class="badge badge-soft-success">View Access</span>
                                                </label>
                                            <?php } if($singleData['edit_access'] == 1){  ?>
                                                <label class="form-check">
                                                    <input type="checkbox" name="role[<?= $singleData['sidebar_url'] ?>][]" <?= isAuthorized('edit', $singleData['sidebar_url'], $perms) ? 'checked' : '' ?> value="edit" id="edit" class="form-check-input">
                                                    <span class="badge badge-soft-success">Edit Access</span>
                                                </label>
                                            <?php } if($singleData['update_access'] == 1){  ?>
                                                <label class="form-check">
                                                    <input type="checkbox" name="role[<?= $singleData['sidebar_url'] ?>][]" <?= isAuthorized('update', $singleData['sidebar_url'], $perms) ? 'checked' : '' ?> value="update" id="update_status" class="form-check-input">
